...
class Vocabulary(object):
	...
	def _build_vocabulary(self):
		if not os.path.isfile(f"{self._data_path[0]}_vocab.txt"):
			...
			with open(f"{self._data_path[0]}_vocab.txt", 'w') as f:
				for w in self.w2i.keys():
					f.write(f"{w}\t{self.w2i[w]}\n")
		else:
			print('restoring vocab...')
			with open(f"{self._data_path[0]}_vocab.txt", 'r') as f:
				lines = f.readlines()
			words = [item.split('\t')[0] for item in lines]
			ids = [item.split('\t')[1][:-1] for item in lines]
			for ii, word in enumerate(words):
				self.w2i[word] = int(ids[ii])
				self.i2w[int(ids[ii])] = word
