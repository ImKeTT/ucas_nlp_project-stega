...
from gpt_2.utils import limit_past, kl, entropy, bits2int, int2bits, is_sent_finish, num_same_from_beg
    ...
    with torch.no_grad():
        ...
        while i < len(message) or (finish_sent and not sent_finish):
            ...
            if i >= len(message):
                ...
                cum_probs = probs_temp_int.cumsum(0) # 计算出每个单词对应概率区间
...
def decode_arithmetic(model, enc, text, context, device='cpu', temp=1.0, precision=16, topk=50000):
    ...
    inp = enc.encode(text) # cover string to word id
