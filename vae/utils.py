	...
	def build_generator(self, batch_size, shuffle=True):
		...
		while True:
			...
			if len(batch_indices) == 0:
				return True
			batch_data = np.array(self._data)[batch_indices]
			input = []
			target = []
			length = []
			for _ in range(batch_data.shape[0]):
				input.append(batch_data[_][:-1])
				target.append(batch_data[_][1:])
				length.append(len(batch_data[_]) - 1)
			input = self._padding(input)
			target = self._padding(target)
			yield input, target, np.array(length)


class Generator_B(object):
	def __init__(self, data, data_raw, vocabulary):
		self._data = data
	...
	def build_generator(self, batch_size, shuffle=True):
		...
		while True:
			...
			if len(batch_indices) == 0:
				return True
			batch_data = np.array(self._data)[batch_indices]
			batch_data_raw = np.array(self._data_raw)[batch_indices]
			input = []
			target = []
			length = []
			for _ in range(batch_data.shape[0]):
			...
			input = self._padding(input)
			target = self._padding(target)
			yield input, target, np.array(length), batch_data_raw
